...
// the unskinned template's raw hex values and top-nav layout). Scoped to
// residential listings only; Plot/Land/PG-specific sub-forms shown in
// the same Figma section are deferred to a later phase.
...
import { Furnishing, ListingType, PROPERTY_TYPE_LABELS } from "@/lib/enums";
import { cn, toOptionalNumber } from "@/lib/utils";
import { propertyInfoSchema, RESIDENTIAL_PROPERTY_TYPES, type PropertyInfoValues } from "@/lib/validation/postProperty";
...
const PROPERTY_TYPE_OPTIONS = RESIDENTIAL_PROPERTY_TYPES.map((value) => ({ value, label: PROPERTY_TYPE_LABELS[value] }));
    ...
    <form onSubmit={onSubmit} className="flex w-full flex-col gap-8">
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-3">
          <AuthTextField label="Bedrooms (BHK)" placeholder="3" register={register("bhk", { setValueAs: toOptionalNumber })} error={errors.bhk?.message} />
          <AuthTextField
            label="Bathrooms"
            placeholder="2"
