...
// token. The access token lives in memory only, never localStorage
// (14_Security.md), so a full page reload always resets to "idle" and
// relies on ensureAuthResolved (lib/auth/session.ts) to restore it from the
